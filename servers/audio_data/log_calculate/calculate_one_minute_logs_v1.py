...
class CalculateOneMinuteLogs(object):
    ...
    def init_cassandra(self):
        if not self.cassandra_connector.prepare_stmt("update_device_audio_data", "ks_ajmd_dw", "update device_audio_daily set start_type=?,end_type=?,end_time=?,live_status=?,pid=?,tid=?,netstat=? where date=? and device_id=? and start_time=? and phid=?"):
